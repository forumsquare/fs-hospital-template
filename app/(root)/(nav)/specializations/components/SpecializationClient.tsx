...
const SpecializationClient = ({
  initialCategories,
  initialProcedures,
  initialFacilities,
  initialDoctors,
}: SpecializationClientProps) => {
  ...
  const [categoryId, setCategoryId] = useState(() => {
    const initialSorted = [...initialCategories].sort((a, b) =>
  ...
  const isFirstCategory =
    activeIndex === 0 && categories && categories.length > 0;

  const { data: procedures, isPending: proceduresPending } =
    useGetProceduresListQuery(categoryId, {
      initialData: isFirstCategory ? initialProcedures : undefined,
    });
  const { data: facilitiesList, isPending: facilitiesPending } =
    useGetFacilitiesQuery(categoryId, {
  ...
  const { data: doctors } = useGetDoctorListQuery({
  ...
  const formattedProcedures =
    (procedures as ProcedureType[])?.map((proc) => ({
      id: proc.id,
      name: proc.name,
      image: proc.image,
    ...
    <section className=" rounded-2xl sm:mt-14 space-y-7 mx-0  px-6 sm:px-10 py-14 sm:mx-auto">
      ...
      <div>
        ...
        <div className="overflow-hidden border-t border-zinc-200 dark:border-zinc-700">
              ...
              <article key={index} className="py-5">
                {activeIndex === index && (
                  <>
                    ...
                    <h4 className="font-bold text-3xl mt-10">Doctors</h4>
          ...
          <h4 className="font-bold text-3xl mt-10">Facilities</h4>
          <p className="text-sm font-sans italic text-muted-foreground  tracking-wide font-medium pb-5">
