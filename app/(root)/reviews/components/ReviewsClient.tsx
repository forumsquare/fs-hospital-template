...
const ReviewsClient: React.FC<ReviewsClientProps> = ({ initialDoctors, initialReviews }) => {
    ...
    const averageRating = useMemo(() => {
        ...
        const sum = reviews.reduce((acc: number, r: UserReviewType) => acc + Number(r.rating), 0);
        ...
        <main className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 md:mb-12">
                <div className="space-y-3 md:space-y-4">
                    <button onClick={() => router.back()} className="flex items-center gap-2 text-primary font-medium hover:opacity-70 transition-opacity">
                        <ArrowLeft className="w-3.5 h-3.5" />
                        <span className="uppercase text-[9px] md:text-[10px] tracking-widest font-bold">Back to Doctors</span>
                    </button>
                    <h1 className="text-3xl md:text-5xl font-bold text-slate-900 tracking-tight">Patient Reviews</h1>
                    <div className="flex flex-wrap items-center gap-3 md:gap-4">
                        <div className="flex items-center gap-1 bg-green-50 px-3 py-1 rounded-full border border-green-100 flex-shrink-0">
                            ...
                            <span className="font-bold text-green-700 text-sm md:text-base">{averageRating}</span>
                            <span className="text-green-600/60 text-xs md:text-sm">/ 5</span>
                        </div>
                        <p className="text-slate-500 font-medium tracking-tight text-sm">Based on {reviews.length}+ experiences</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <aside className="lg:col-span-4 space-y-8">
                    ...
                    <section className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm">
                        <h3 className="font-bold text-xl text-slate-900 mb-6">Review Breakdown</h3>
                        <div className="space-y-4">
                            {[5, 4, 3, 2, 1].map((star, i) => (
                                <div key={star} className="flex items-center gap-4">
                                    <span className="text-sm font-bold text-slate-600 w-12">{star} Star</span>
                                    ...
                                    <span className="text-sm font-bold text-slate-400 w-8">{stats[i]}%</span>
                                </div>
                            ))}
                        </div>
                    </section>
                    ...
                    <section className="bg-slate-50 p-8 rounded-2xl border border-slate-100/50">
                        <div className="flex items-center gap-2 mb-6">
                            ...
                            <h3 className="font-bold text-xl text-slate-900">Refine Results</h3>
                        </div>

                        <div className="space-y-6">
                            {/* Doctor Select */}
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Filter by Doctor</label>
                                <Select value={selectedDoctor || "ALL"} onValueChange={handleDoctorChange}>
                            ...
                            {/* Sort Options */}
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Sort Reviews</label>
                                <div className="flex flex-col gap-2">
                                    <button
                                        onClick={() => { setSortBy("DATE"); setSortOrder("desc"); setCurrentPage(1); }}
                                        className={cn(
                                    ...
                                    >
                                        ...
                                        {sortBy === "DATE" && sortOrder === "desc" && <Star className="w-4 h-4 fill-primary text-primary" />}
                                    </button>

                                    <button
                                        onClick={() => { setSortBy("RATING"); setSortOrder("desc"); setCurrentPage(1); }}
                                        className={cn(
                                            "flex items-center justify-between p-3 rounded-xl transition-all duration-200 border-2 text-left",
                                    ...
                                    >
                                        ...
                                        {sortBy === "RATING" && sortOrder === "desc" && <Star className="w-4 h-4 fill-primary text-primary" />}
                                    </button>

                                    <button
                                        onClick={() => { setSortBy("RATING"); setSortOrder("asc"); setCurrentPage(1); }}
                                        className={cn(
                                            "flex items-center justify-between p-3 rounded-xl transition-all duration-200 border-2 text-left",
                                    ...
                                    >
                                        ...
                                        {sortBy === "RATING" && sortOrder === "asc" && <Star className="w-4 h-4 fill-primary text-primary" />}
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <div className="lg:col-span-8 space-y-6">
                    {reviewsPending ? (
                        <div className="flex flex-col gap-6">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="h-64 bg-slate-50 animate-pulse rounded-2xl border border-slate-100"></div>
                            ))}
                        </div>
                    ) : (
                        <>
                            ...
                            {reviews.length === 0 && (
                                <div className="text-center py-24 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                                    ...
                                    <h3 className="text-slate-900 font-bold text-xl">No reviews yet</h3>
