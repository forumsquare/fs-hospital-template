...
>(({ side = 'bottom', className, children, ...props }, ref) => (
  <ResponsiveModalPortal >
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(ResponsiveModalVariants({ side }), className , "rounded-t-2xl bg-gradient-to-b from-[#f7f5f2] via-[#dfdfde] to-[#f7f5f2] backdrop-blur-md border-none  px-6  ")}
      {...props}
    >
      {children}
      <ResponsiveModalClose className="absolute right-4 top-4 rounded-2xl opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-neutral-300">
        <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn('text-2xl font-bold  txt-shadow', className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description
    ref={ref}
    className={cn('text-sm text-neutral-700 font-sans italic font-medium tracking-wider leading-5', className)}
    {...props}
  />
));
